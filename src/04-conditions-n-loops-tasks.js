...
function findFirstSingleChar(str) {
  const arr = str.split('');
  const repeated = new Array(arr.length).fill(false);
...
function isCreditCardNumber(ccn) {
  ...
  return (10 - (sum % 10)) % 10 === parseInt(arr[0], 10);
}
...
function isBracketsBalanced(str) {
  ...
  const regex = /(\[\])|(\(\))|(\{\})|(<>)/g;
  let reducedStr = str.replace(regex, '');

  while (reducedStr !== str) {
    str = reducedStr;
    reducedStr = str.replace(regex, '');
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  // Разделяем каждый путь на части
  const splitPathes = pathes.map((path) => path.split('/'));

  // Находим минимальную длину массива частей
  const minLength = Math.min(...splitPathes.map((parts) => parts.length));

  // Инициализируем общий префикс
  const commonPrefix = [];

  // Проходим по каждой части путей
  for (let i = 0; i < minLength; i += 1) {
    // Получаем i-ю часть каждого пути
    const part = splitPathes[0][i];

    // Проверяем, совпадает ли эта часть во всех путях
...
function getMatrixProduct(m1, m2) {
  ...
  if (colsM1 !== rowsM2) {
    throw new Error('Невозможно умножить матрицы: количество столбцов первой матрицы не равно количеству строк второй матрицы');
  }
